...
We've ejected from Create React App, so this document is most applicable. Refer to it over the current Create React App documentation. In addition, this repository uses PostCSS for CSS precompilation. Documentation for this is coming, in the mean time, refrence the applicable PostCSS package.

We use [React Storybook](https://github.com/storybooks/storybook) for isolated component development. To start run:

`npm start storybook`

This will start the storybook server at `localhost:6006`. When creating a component, be sure to include a `component.stories.js` file.
